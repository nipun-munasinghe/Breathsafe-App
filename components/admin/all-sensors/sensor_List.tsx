...
const SensorList: React.FC<SensorListProps> = ({
    ...
    const handleDownloadPDF = () => {
        ...
        doc.text("Sensor List Report", 14, 20);
        ...
        doc.text(`Total Sensors: ${sensors.length} | Online: ${onlineSensors} | Active: ${activeSensors}`, 14, 35);

        // Prepare table data
        const tableData = sensors.map(sensor => [
            sensor.id.toString(),
            sensor.name,
            sensor.location,
            sensor.status,
        ...
        autoTable(doc, {
            startY: 42,
            head: [['ID', 'Name', 'Location', 'Status', 'Active', 'Latitude', 'Longitude', 'Install Date', 'Battery']],
            body: tableData,
            theme: 'striped',
            headStyles: {
                fillColor: [34, 197, 94],
        ...
        // Save the PDF
        doc.save(`sensor-list-${new Date().toISOString().split('T')[0]}.pdf`);
    };
                ...
                <div className="bg-gradient-to-r from-lime-600 to-emerald-800 p-8">
                    <div className="flex items-center justify-between">
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-white text-lime-700 rounded-lg hover:bg-lime-50 transition-colors font-semibold shadow-lg z-50"
                        >
